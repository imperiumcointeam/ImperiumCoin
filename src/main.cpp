...
uint256 hashGenesisBlock("0xdd61df524d6d0f050894068653c6c14d6a706a6487e6b97548c94fa41309bbaa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Imperium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Imperium Signed Message:\n";
...
{
    // Imperium: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nRulesAHeight = 11500;
static const int64 nATargetTimespan = 20 * 60; // 20min - 20 blocks
static const int64 nATargetSpacing = 1 * 60;
static const int64 nAInterval = nATargetTimespan / nATargetSpacing;
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    if (fTestNet && nTime > nATargetSpacing*2)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    else
    {
        ...
        if ((pindexLast->nHeight+1) % nAInterval != 0 && pindexLast->nHeight != nRulesAHeight)
        {
            ...
            if (fTestNet)
            {
                ...
                if (pblock->nTime > pindexLast->nTime + nATargetSpacing*2)
                    return nProofOfWorkLimit;
                else
                {
	...
	if (pindexLast->nHeight != nRulesAHeight)
	{
	    blockstogoback = pindexLast->nHeight - 11000;
	}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Imperium: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Imperium: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdd61df524d6d0f050894068653c6c14d6a706a6487e6b97548c94fa41309bbaa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1410882805;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087227549;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ImperiumCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ImperiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ImperiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ImperiumCoinMiner(CWallet *pwallet)
{
    printf("ImperiumMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ImperiumCoin-miner");
    ...
    try { loop {
        ...
        printf("Running Imperium with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ImperiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ImperiumCoinMiner, pwallet));
}
